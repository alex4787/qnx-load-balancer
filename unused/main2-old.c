...
//	printf("Migrating [pid=%d tid=%d] to CPU %d\n", min_task.pid, min_task.tid, MinCpu);
//	int slay_pid;
//
...
//
//	slay_pid = spawnlp(P_WAIT, "slay", "slay", "-i", "-C", min_cpu, "-T", min_task_tid, min_task_pid, NULL);
//
//	if (slay_pid == -1) {
...
////							if (threadinfo.state == STATE_READY || threadinfo.state == STATE_RUNNING)
...
////								printf("\ttid=%d cpu=%d state=%d\n", tid, cpu, threadinfo.state);
//
//							// A thread created by the system
////							if (cpu != 0) {
////								// System thread is not in CPU 0
////								perform_migration(threadinfo, 0);
////							}
//						}
//					}
//				}
...
//};
//
//void * thread_run_sleep(void *_args) {
//	struct arg_struct *args = (struct arg_struct *)_args;
//	printf("CPU %d\n", args->cpu);
////	ThreadCtl( _NTO_TCTL_RUNMASK, (void *)args->cpu);
//
//	unsigned    num_elements = 0;
//	int         *rsizep, masksize_bytes, size;
//	unsigned    *rmaskp, *imaskp;
...
////		diff = (after.tv_nsec + (after.tv_sec * 1000000)) - (before.tv_nsec + (before.tv_sec * 1000000));
//////		diff = (after.tv_nsec + (after.tv_sec * 1000000));
////
////		printf("[%d] %lu\n", gettid(), diff);
////		if (diff >= usec)
////			break;
...
//	}
//
//	arg_structs[0].msec = 1000;
//	arg_structs[0].cpu = 1;
//	pthread_create(threads + 0, thread_attrs + 0, thread_run_sleep, (void *) (arg_structs));
//
//	arg_structs[1].msec = 500;
//	arg_structs[1].cpu = 2;
//	pthread_create(threads + 1, thread_attrs + 1, thread_run_sleep, (void *) (arg_structs + 1));
//
//	arg_structs[2].msec = 500;
//	arg_structs[2].cpu = 3;
//	pthread_create(threads + 2, thread_attrs + 2, thread_run_sleep, (void *) (arg_structs + 2));
//	run_load_balancer();
//
//	arg_structs[3].msec = 500;
//	arg_structs[3].cpu = 1;
//	pthread_create(threads + 3, thread_attrs + 3, thread_run_sleep, (void *) (arg_structs + 3));
//	run_load_balancer();
//
//	arg_structs[4].msec = 500;
//	arg_structs[4].cpu = 1;
//	pthread_create(threads + 4, thread_attrs + 4, thread_run_sleep, (void *) (arg_structs + 4));
//	run_load_balancer();
//
//	arg_structs[5].msec = 500;
//	arg_structs[5].cpu = 1;
//	pthread_create(threads + 5, thread_attrs + 5, thread_run_sleep, (void *) (arg_structs + 5));
//
